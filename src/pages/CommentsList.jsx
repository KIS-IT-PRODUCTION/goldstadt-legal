    ...
    try {
      ...
      if (response.ok) {
        ...
        const grouped = flatComments.reduce((acc, comment) => {
          ...
          if (!acc[newsId]) {
            acc[newsId] = { id: newsId, title: comment.newsTitle || 'Статья без названия', comments: [] };
          }
        ...
        <div className="section-header-inner" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%', flexWrap: 'wrap', gap: '16px' }}>
          <div>
            <div className="section-h">
              Модерация по статьям
              {!loading && (
                <span className="table-count">{totalCommentsCount} комментариев</span>
              )}
            </div>
            <div className="section-sub">Комментарии сгруппированы по материалам</div>
          </div>
          ...
          <div style={{ maxWidth: '350px', width: '100%' }}>
            <input 
              ...
              placeholder="🔍 Искать статью, текст или автора..." 
              value={searchQuery}
              ...
              <table className="admin-table" style={{ margin: 0, background: 'transparent', border: 'none' }}>
                <thead>
                  <tr style={{ background: 'transparent' }}>
                    <th style={{ paddingLeft: '20px', width: '25%' }}>Пользователь</th>
                    <th style={{ width: '55%' }}>Текст комментария</th>
                    <th style={{ width: '15%' }}>Дата</th>
                    <th style={{ textAlign: 'right', paddingRight: '20px', width: '5%' }}>Действия</th>
                  </tr>
                </thead>
                <tbody>
                  {newsItem.comments.map((comment) => (
                    <tr key={comment._id} style={{ background: 'transparent' }}>
                      <td style={{ paddingLeft: '20px' }}>
                        <div style={{ fontWeight: 500, color: '#fff', fontSize: '13px' }}>{comment.user?.name || 'Аноним'}</div>
                      ...
                      <td style={{ color: 'rgba(255,255,255,0.35)', fontSize: '12px', whiteSpace: 'nowrap' }}>
                        {comment.createdAt ? new Date(comment.createdAt).toLocaleString('ru-RU', { dateStyle: 'short', timeStyle: 'short' }) : '---'}
                      </td>
